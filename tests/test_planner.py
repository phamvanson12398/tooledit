...
def test_music_base_minus_5db_and_duck_nodes(tmp_path):
    from app.planner.builder import db_to_gain
    from app.styles import load_style
    ...
    values = {round(v, 4) for k in seg["common_keyframes"] for kf in k["keyframe_list"] for v in kf["values"]}
    assert abs(db_to_gain(-5) - 0.5623) < 1e-4
